...
def create_employee_table(cur, conn):
    ...
    cur.execute("CREATE TABLE employees (employee_id INTEGER, first_name TEXT, last_name TEXT, hire_date TEXT, job_id INTEGER, salary INTEGER)")
    conn.commit()
...
def add_employee(filename, cur, conn):
    ...
    for employee in employee_data:
        cur.execute("INSERT INTO employees VALUES(?,?,?,?,?,?)", (employee["employee_id"], employee["first_name"], employee["last_name"], employee["hire_date"], employee["job_id"], employee["salary"]))
    conn.commit()

# TASK 2: GET JOB AND HIRE_DATE INFORMATION
def job_and_hire_date(cur, conn):
    cur.execute("SELECT hire_date, job_title FROM employees JOIN jobs ON employees.job_id = jobs.job_id")
    results = sorted(cur.fetchall(), key=lambda x:x[0])
    return results[0][1]
...
def visualization_salary_data(cur, conn):
    cur.execute("SELECT job_title FROM employees JOIN jobs ON employees.job_id = jobs.job_id")
    x = [x[0] for x in cur.fetchall()]
    cur.execute("SELECT salary FROM employees JOIN jobs ON employees.job_id = jobs.job_id")
